...
export function SeasonsPanel({
  ...
  /** 추가/수정 폼을 담은 모달이 열려 있는지 — editingId 와 별개다. editingId는
   *  "폼에 채워진 게 새 항목인지 기존 항목인지"를 가리키고, 이건 "폼 자체를
   *  지금 보여줄지"를 가리킨다. */
    ...
    <div className="flex flex-col gap-6">
      <section className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            onClick={openNewForm}
            className="rounded-lg border border-dashed border-cream-deep px-4 py-2 text-sm font-bold text-ink-500 transition-colors hover:border-cheese-300 hover:text-ink-700"
          >
            + {labels.seasonNoun} 추가
          </button>
        </div>
      ...
      {modalOpen && (
        <Modal
          title={editingId ? `${labels.seasonNoun} 수정` : `새 ${labels.seasonNoun} 추가`}
          onClose={closeModal}
        >
...
export function ActivitiesPanel({
  ...
  /** 추가/수정 폼을 담은 모달이 열려 있는지 — SeasonsPanel과 같은 이유로
   *  editingId 와 별개로 둔다. */
  const [modalOpen, setModalOpen] = useState(false)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {modalOpen && (
        <Modal
          title={editingId ? `${labels.activityNoun} 수정` : `새 ${labels.activityNoun} 만들기`}
          onClose={closeModal}
          wide
        >
  ...
  return (
    ...
    >
      ...
      <label className="flex items-center gap-1.5 text-xs font-semibold text-ink-600">
        ...
        />
        이미지·동영상(mp4)·PDF·한글·워드·PPT·엑셀·CSV 파일 첨부
      </label>
    ...
    <div className="flex flex-col gap-1.5 rounded-lg border border-dashed border-cream-deep bg-cream/40 p-2.5">
      {meta ? (
        ...
        <input
          ref={inputRef}
          type="file"
          accept=".png,.jpg,.jpeg,.gif,.webp,.mp4,.pdf,.hwp,.hwpx,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.csv"
          disabled={busy}
          onChange={(event) => {
