...
- `pico_센서_확장_구현_계획.md` — 부품을 5종에서 14종으로 늘린 2차 설계 문서.
  ADC → PWM → 디지털 전용 → 별도 모듈 순서와 그 근거, 아직 안 한 것(7세그먼트,
  DHT11, 네오픽셀, 초음파, I2C LCD)과 위험한 것(초음파의 busy-wait 문제)이 적혀
  있다. **1·2단계와 3단계 대부분이 구현·배포됨.**
...
| Pico 2 W 실습 | `/practice/pico` | **회로 / 코드 탭 전환**(`PicoLab.tsx`, 콘솔은 코드 탭 안). 부품 14종 + 되돌리기 + 부저 실제 소리. 아래 "Pico 시뮬레이터 확장" 참고. `PicoGate.tsx`가 교사/학생(설정 공개 여부)로 게이트. Monaco는 진입 시 동적 로드 |
...
**부품 14종.** 출력 = LED(PWM이면 밝기), RGB LED(PWM이면 색 혼합), 부저(Web Audio로
실제 소리 + 음소거), 서보(뿔이 실제로 돌아감), 릴레이, 진동모터, 신호등.
입력 = 버튼, 스위치, PIR, 틸트, 리드 스위치, 가변저항(노브), 조도센서(슬라이더).
...
**아직 안 한 것**: 7세그먼트(3단계 마지막), DHT11, 네오픽셀, 초음파, I2C LCD.
초음파는 위험하다 — 실제 MicroPython 패턴이 `while echo.value()==0:` busy-wait라
`time.sleep`에 `await`를 주입하는 실시간 모드와 부딪힌다. 붙이기 전에 최소 재현으로
먼저 검증할 것.

**검증 안 된 것**: 아이패드(터치). 오늘 늘어난 조작부(렌즈 돔, 유리관, 슬라이더 손잡이,
노브, 전선 끝 손잡이 r=7)가 전부 마우스 기준이라 손가락으로 눌러본 적이 없다.
